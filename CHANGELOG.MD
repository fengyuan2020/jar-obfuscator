...
- [BUG] 修复接口方法混淆兼容性 @4ra1n
- [BUG] 修复 `Spring Boot` 与 `WAR` 字符串解密输出 @4ra1n
- [优化] 花指令保留 `Frame` 并扩展至五级 @4ra1n
- [优化] 移除 `jar-obf-lib` 无需 `CLASSPATH` @4ra1n
- [测试] 新增 `JDK 8 JDK 17 JDK 25` 全量测试 @4ra1n

## 2.0.1

更新日志：

- [SECURITY] 修复可能存在的 `Zip Slip` 安全问题 @4ra1n
- [功能] 新增外部依赖 API 保护，自动保留实现/覆盖外部父类和接口的方法名 @4ra1n
- [功能] 新增内置框架保护规则，默认保护 `Spring/MyBatis/JPA/Jackson/Servlet/JAX-RS` 等反射入口 @4ra1n
- [功能] 支持自定义注解的混淆 @4ra1n
- [BUG] 修复方法名混淆时继承、接口和覆写方法映射不一致的问题 @4ra1n
- [BUG] 修复单独开启方法名或字段名混淆时不生效的问题 @4ra1n
- [BUG] 修复多次运行时全局状态未清理导致映射污染的问题 @4ra1n
- [BUG] 修复部分 `class` 转换失败后仍继续生成 `jar` 的问题 @4ra1n
- [优化] 安全加固检查所有的删除点确保不会误删文件 @4ra1n
- [优化] 混淆类名时同步重写 `MANIFEST.MF` 和`META-INF/services` 等资源文件 @4ra1n
- [优化] 泛型 `Signature` 使用 `ASM SignatureReader` 重写避免漏改 @4ra1n
- [BUG] 修复内部类包含 `$` 导致的问题 @4ra1n
- [BUG] 修复 `UTF-8` 字符串过长问题 @4ra1n
- [BUG] 修复字段名混淆未跳过黑名单类导致第三方库（如 `jsoup`）字段引用不一致的问题 @4ra1n
- [BUG] 修复方法名混淆未跳过黑名单类导致第三方库方法引用不一致的问题 @4ra1n
- [BUG] 修复枚举类被完全跳过导致其方法体内对其他类的字段/方法引用未被更新的问题 @4ra1n
- [BUG] 修复子类引用继承的字段时因 `owner` 为子类而未被重命名的问题 @4ra1n
